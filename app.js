...
function genInforme(){
  ...
  const isin=p=>st.instrumentos[p.inst]?.isin||'';
  const compRows=prod.map(p=>`<tr><td>${p.nombre}</td><td class="isin">${isin(p)}</td><td class="num">${e0(p.valor)}</td><td class="num">${p1(p.pct)}</td></tr>`).join('');
  const p1html=`<div class="page">${HT('Cierre 31/08/2026')}
    ...
    <div class="cols2">
      <div>${secbar('Composición actual','Distribución por instrumentos')}<div class="panel"><table><thead><tr><th>Instrumento</th><th>ISIN</th><th class="num">Valor €</th><th class="num">Peso</th></tr></thead><tbody>${compRows}</tbody></table></div></div>
      <div>${secbar('Distribución por pesos','Peso de cada instrumento')}<div class="panel pc">${donut(prod.map(p=>({n:p.nombre,v:p.valor})))}</div></div></div>
    ${secbar('Evolución del año','Rentabilidad acumulada 2026')}<div class="panel">${linea(M)}</div>${FT(1)}</div>`;
  ...
  const p2html=`<div class="page">${HT('Histórico anual')}
    ${secbar('Detalle anual','Aportaciones, retiradas y resultado')}<div class="panel"><table class="anual"><thead><tr><th>Año</th><th class="num">Inicio €</th><th class="num">Fin €</th><th class="num">Aport. €</th><th class="num">Retir. €</th><th class="num">Resultado €</th><th class="num">Rent.</th><th>Estado</th></tr></thead><tbody>${arows||'<tr><td colspan="8" class="muted">Sin histórico anual.</td></tr>'}</tbody></table>${A.length?`<p class="riesgo">Volatilidad anual: <b>${p1(stdev(A.map(y=>y.rent||0)))}</b> · Caída máxima: <b>${p1(maxDD(A.map(y=>y.rent||0)))}</b></p>`:''}</div>
    <div class="cols2"><div>${secbar('Rentabilidad mensual 2026')}<div class="panel">${barras(MES,M)}</div></div><div>${secbar('Rentabilidad anual')}<div class="panel">${barras(A.map(y=>y.a),A.map(y=>y.rent||0))}</div></div></div>${FT(2)}</div>`;
  ...
  const p3html=`<div class="page">${HT('Operaciones y movimientos')}
    ${secbar('Operaciones desde inicio')}<div class="panel"><table><thead><tr><th>Fecha</th><th>Operación</th><th>Instrumento</th><th class="num">Importe €</th><th class="num">Precio</th><th class="num">Títulos</th></tr></thead><tbody>${orows||'<tr><td colspan="6" class="muted">Sin operaciones.</td></tr>'}</tbody></table></div>
    <div class="cols2"><div>${secbar('Aportaciones y disposiciones')}<div class="panel"><div class="card2"><div class="r2"><span>Aportaciones 2026</span><b>${e0(apPer)}</b></div><div class="r2"><span>Disposiciones 2026</span><b>${e0(rePer)}</b></div><div class="r2"><span>Aportación inicial (inicio de la relación)</span><b>${e0(apInicial)}</b></div><div class="r2"><span>Aportaciones totales desde inicio</span><b>${e0(apTot)}</b></div><div class="r2"><span>Disposiciones desde inicio</span><b>${e0(reTot)}</b></div></div></div></div>
      <div>${secbar('Aviso legal')}<div class="panel"><div class="aviso2">${AVISO}</div></div></div></div>${FT(3)}</div>`;
