...
function donutSVG(segs){const tot=segs.reduce((s,x)=>s+x.v,0)||1,r=78,C=2*Math.PI*r;let off=0,g='';const col=['#8E7577','#E5D0D2','#575755','#B79A9C','#C9B9BA','#7d6a6b','#ddc8c9','#9c8788'];
  segs.forEach((sg,i)=>{const d=sg.v/tot*C;g+=`<circle cx="100" cy="100" r="${r}" fill="none" stroke="${col[i%col.length]}" stroke-width="34" stroke-dasharray="${d} ${C-d}" stroke-dashoffset="${-off}" transform="rotate(-90 100 100)"/>`;off+=d;});
  const leg=segs.map((sg,i)=>`<div class="it"><span class="sw" style="background:${col[i%col.length]}"></span>${sg.n} <b>${pct(sg.v/tot)}</b></div>`).join('');
  return `<div class="donut"><svg viewBox="0 0 200 200" style="width:210px;flex:none">${g}</svg><div class="leg">${leg}</div></div>`;}
function renderAnalisis(){
  ...
  $('view').innerHTML=`<h1>Análisis de la cartera</h1>
    ...
    <div class="card"><h2>Patrimonio por producto</h2>
      ...
      <div style="display:flex;justify-content:center;gap:34px;margin:26px 0 4px;padding-top:22px;border-top:1px solid var(--linea)">${prod.length?donutSVG(prod.slice(0,8).map(p=>({n:p.nombre,v:p.valor}))):''}</div></div>
...
function genInforme(){
  ...
  const compTable=`<table class="comp"><thead><tr><th>Activo</th><th class="num">VL compra</th><th class="num">Valor inicial</th><th class="num">Valor actual</th><th class="num">Dif. €</th><th class="num">Dif. %</th><th class="num">Peso</th></tr></thead><tbody>${compRows}${compTot}</tbody></table>`;
  ...
  const p3html=`<div class="page">${HT('Operaciones y movimientos')}<div class="body">
    <div class="grow col">${secbar('Operaciones desde inicio')}<div class="panel"><table class="ops"><thead><tr><th>Fecha</th><th>Operación</th><th>Instrumento</th><th class="num">Importe €</th><th class="num">Precio</th><th class="num">Títulos</th></tr></thead><tbody>${orows||'<tr><td colspan="6" class="muted">Sin operaciones.</td></tr>'}</tbody></table></div></div>
